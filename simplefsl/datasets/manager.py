...
class FewShotBRSET(Dataset):
    def __init__(self, img_ids, labels, transforms=None):
        ...
        self.labels = labels
        self.img_dir = '/home/victornasc/Metodos-FSL/BRSET/data/imgs'
...
class BRSETManager():
    def __init__(self, training_classes, test_classes, n_shots, n_ways, mean, std, augment = None, batch_size = 25, remove_img = None, seed=None):
        self.training_classes = training_classes
        self.test_classes = test_classes
        self.n_shots = n_shots
        self.n_ways = n_ways
        self.augment = augment
        self.data = pd.read_csv('/home/victornasc/Metodos-FSL/BRSET/data/clean.csv')
    ...
    def get_train_task(self):
        if self.seed != None: 
            seed = self.seed + self.train_task_count
            random.seed(seed)
        else:
            seed = None 
        ...
        for clss in classes:
            objs = self.data[self.data[clss] == 1].sample(n=self.n_shots, random_state = seed)
    ...
    def get_eval_task(self, train_classes = False):
        ...
        for clss in classes:
            objs = self.data[self.data[clss] == 1].sample(n=self.n_shots+1, random_state=seed)
...
if __name__ == "__main__":
    ...
    n_shots = 5
    n_ways = 2
    manager = BRSETManager(training_classes, test_classes, n_shots, n_ways, (0.5, 0.5, 0.5), (0.5, 0.5, 0.5), augment = "basic", batch_size = 1, seed=43)
